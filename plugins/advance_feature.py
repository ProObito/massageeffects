...
# ==================== 1. ADVANCED FORCE SUBSCRIBE CHANNELS ====================

@Client.on_message(filters.command('add_fsub') & filters.private & filters.user(int(OWNER_ID)))
async def add_forcesub(client: Client, message: Message):
    pro = await message.reply("<b><i>Pʀᴏᴄᴇssɪɴɢ....</i></b>", quote=True)
    check = 0
    channel_ids = await obito.get_all_channels()
    ...
    if check == len(fsubs):
        for id in fsubs:
            await obito.add_channel(int(id))
...
@Client.on_message(filters.command('del_fsub') & filters.private & filters.user(int(OWNER_ID)))
async def delete_all_forcesub(client: Client, message: Message):
    ...
    if len(fsubs) == 1 and fsubs[0].lower() == "all":
        if channels:
            ...
            return await pro.edit("<b><blockquote>⁉️ Nᴏ Cʜᴀɴɴᴇʟ ɪᴅ ᴀᴠᴀɪʟᴀᴠʟᴇ ᴛᴏ Dᴇʟᴇᴛᴇ</blockquote></b>", reply_markup=reply_markup)
...
# ==================== 2. BOT ADMINS MANAGEMENT ====================

@Client.on_message(filters.command('add_admins') & filters.private & filters.user(int(OWNER_ID)))
...
@Client.on_message(filters.command('del_admins') & filters.private & filters.user(int(OWNER_ID)))
...
@Client.on_message(filters.command('admin_list') & filters.private & filters.user(int(OWNER_ID)))
...
# ==================== 3. BANNED USERS FUNCTION MANAGEMENT ====================

@Client.on_message(filters.command('add_banuser') & filters.private & is_admin)
async def add_banuser(client: Client, message: Message):        
    pro = await message.reply("<b><i>Pʀᴏᴄᴇssɪɴɢ....</i></b>", quote=True)
    check, autho_users = 0, []
    banuser_ids = await obito.get_ban_users()
    autho_users = await obito.get_all_admins()
    autho_users.append(int(OWNER_ID))
    banusers = message.text.split()[1:]

    reply_markup = InlineKeyboardMarkup([[InlineKeyboardButton("Cʟᴏsᴇ ✖️", callback_data="close")]])
    
    if not banusers:
        return await pro.edit("<b>Yᴏᴜ ɴᴇᴇᴅ ᴛᴏ ᴀᴅᴅ Bᴀɴɴᴇᴅ Usᴇʀ ɪᴅs\n<blockquote><u>EXAMPLE</u> :\n/add_banuser [user_id] :</b> ʏᴏᴜ ᴄᴀɴ ᴀᴅᴅ ᴏɴᴇ ᴏʀ ᴍᴜʟᴛɪᴘʟᴇ ᴜsᴇʀ ɪᴅ ᴀᴛ ᴀ ᴛɪᴍE.</blockquote>", reply_markup=reply_markup)
    ...
    if check == len(banusers):
        for id in banusers:
            await obito.add_ban_user(int(id))
...
async def delete_banuser(client: Client, message: Message):        
    ...
    if len(banuser_ids) >= 1:
        ...
        await pro.edit(f"<b>⛔️ Pʀᴏᴠɪᴅᴇᴅ Bᴀɴɴᴇᴅ Usᴇʀ ɪᴅs ᴀʀᴇ Dᴇʟᴇᴛᴇᴅ :\n\n{passed}</b>", reply_markup=reply_markup)
